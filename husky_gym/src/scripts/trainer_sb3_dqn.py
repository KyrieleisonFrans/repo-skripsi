...
from config import DQN_PARAMS, TOTAL_TIMESTEPS
from gym_env import GymEnv 


class DirectoryManager:
    def __init__(self, base_ws_dir="/home/husky_ws/src/husky_gym/src/ddqn_husky/"):
        self.base_ws_dir = os.path.join(base_ws_dir, "dqn_sb3")
        self.project_dirs = {}
        
    def setup_dir(self, project_name="dqn_sb3"):
    ...
    def _check_latest_training_status(self, parent_dir, base_name):
        ...
        checkpoint_path = os.path.join(latest_dir_path, f"husky_dqn_checkpoint_300000.zip")
        ...
        if os.path.exists(eval_path):
            try:
                ...
                if len(data['timesteps']) > 0 and data['timesteps'][-1] >= 300000:
                    return latest_dir, True
            except:
                pass
...
def setup_ros_node():
    if not rospy.core.is_initialized():
        rospy.init_node('husky_dqn_training_node', anonymous=True, log_level=rospy.INFO)
        rospy.loginfo("ROS training node 'husky_dqn_training_node' initialized.")
    else:
...
def setup_model_and_callbacks(env, eval_env, dir_manager):
    ...
    model = DQN(
        ...
        max_grad_norm=DQN_PARAMS["max_grad_norm"],
        tensorboard_log=dir_manager.project_dirs['tensorboard'],
        verbose=DQN_PARAMS["verbose"],
    )

    # Callbacks
    stop_callback = StopTrainingOnRewardThreshold(reward_threshold=3.0, verbose=1)
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="husky_dqn_checkpoint",
...
def train_dqn_agent():
    ...
    dirs = dir_manager.setup_dir(project_name="dqn_sb3")
    ...
    remaining_timesteps = 300000

    model_dir = dirs['model']
    if os.path.exists(model_dir):
        checkpoints = []
        for f in os.listdir(model_dir):
            if f.startswith("husky_dqn_checkpoint") and f.endswith(".zip"):
                try:
        ...
        if checkpoints:
            ...
            remaining_timesteps = max(0, 300000 - completed_timesteps)
    ...
    # Train
    rospy.loginfo("Starting DQN training...")
    try:
        model.learn(
